...
export const S = {
  ...
  floatCard: (_hovered: boolean): React.CSSProperties => ({
...
export function Sidebar({ dark, onToggleDark, activeView, onCreate, onGoLibrary }:
  ...
  return (
    <>
      <aside className="w-60 flex-shrink-0 h-full flex flex-col relative z-40" style={S.sidebar}>
        ...
        <SidebarSection label="Apariencia">
          <button onClick={onToggleDark}
            className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted transition-colors rounded-md"
...
export function PrimaryButton({ onClick, icon, label, disabled = false }: { onClick?: () => void; icon: React.ReactNode; label: string; disabled?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled}
      className={`btn-primary-wrap flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold transition-all active:scale-95 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90'}`}
